...
## 1. Proxy Usage
If you need to route your crawl traffic through a proxy—whether for IP rotation, geo-testing, or privacy—Crawl4AI supports it via ```python
...
You can sign in once, export the browser context, and reuse it later—without re-entering credentials.
  * **
```await context.storage_state(path="my_storage.json")```python
**: Exports cookies, localStorage, etc. to a file.
  * Provide 
```storage_state="my_storage.json"```python
...
```~/.crawl4ai/robots/robots_cache.db```python
